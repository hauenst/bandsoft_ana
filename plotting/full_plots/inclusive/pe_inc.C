void pe_inc(TString inDat, TString inSim, int smeared){
	...
	if (smeared!=0) {plotsmeared = true; cout << "we are using smearing" << endl;}
	...
	for( int i = 0 ; i < 1 ; i++){
		...
		if (plotsmeared)
		{
			inTreeSim->Draw(Form("eHit_smeared->getMomentum() >> pe_sim_%i",i),inclusive_smeared);
		}
		else {
	...
	pe_dat[0]->SaveAs("data_pe_inc.root");
	pe_sim[0]->SaveAs("sim_pe_inc.root");
	return;
}
